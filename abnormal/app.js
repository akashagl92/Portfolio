// Custom highlights for Abnormal Security
const ABNORMAL_HIGHLIGHTS = {
    role: "AI Product Builder",
    company: "Abnormal Security",
    themes: ["AI-Native Productivity", "GTM Transformation", "Agentic Workflows", "Multi-Agent Coordination"]
};
        ...
        document.querySelectorAll('.project-card[data-repo]').forEach(card => {
            ...
            if (data && data.ai_summary) {
                ...
                if (descEl) {
                    ...
                    // Add a subtle badge
                    // descEl.insertAdjacentHTML('afterbegin', '<span style="font-size:0.7em; opacity:0.7; display:block; margin-bottom:4px;">✨ AI Summary</span>');
                }
                ...
                if (data.ai_tags && data.ai_tags.length > 0) {
                    ...
                    if (tagsContainer) {
                        // Keep the first "highlight-tag" if it exists, replace others? 
                        // Or just completely replace? User asked to "use these summaries", implying tags too.
                        // Let's replace but try to preserve the style of the first one if we can, or just standard spans
                ...
                // Optional: Add Complexity Score tooltip or badge?
                // For now, keep it clean.

                // Add "AI Verified" badge to card
                card.style.position = 'relative';
                // card.insertAdjacentHTML('beforeend', '<div style="position:absolute; top:10px; right:10px; font-size:12px; opacity:0.5;" title="Summary generated by AI Council">✨</div>');
            }
        });

        console.log('✨ Agentic summaries loaded');
    } catch (e) {
